...
from transformers.utils import is_flash_attn_greater_or_equal_2_10, is_torch_fx_available, logging

from model.anemone_config import AnemoneConfig
from layers.jetmoe.utils.moe import MoE
...
class AnemoneAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: AnemoneConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.top_k = config.attn_top_k
        self.kv_projection_size = self.num_key_value_heads * self.head_dim
        self.is_causal = True
        self.attention_dropout = config.attention_dropout

        assert self.num_heads == self.num_key_value_heads * config.attn_top_k
    ...
    def forward(
        ...
        query_states, aux_loss = self.moah.map(hidden_states)
        ...
        key_states = repeat_kv(key_states, self.top_k)
        value_states = repeat_kv(value_states, self.top_k)
        ...
        attn_output = self.moah.reduce(attn_output)
...
class AnemoneFlashAttention2(AnemoneAttention):
    """
    ...
    def forward(
        ...
        # attn_output = attn_output.reshape(bsz, q_len, self.top_k, self.kv_projection_size)
        attn_output = attn_output.reshape(bsz, q_len, self.top_k, self.kv_projection_size).contiguous()
        attn_output = self.moah.reduce(attn_output)
...
class AnemoneSdpaAttention(AnemoneAttention):
    """
...
JAMBA_ATTENTION_CLASSES = {
    "eager": AnemoneAttention,
    "flash_attention_2": AnemoneFlashAttention2,
    "sdpa": AnemoneSdpaAttention,
}